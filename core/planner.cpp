...
static int tissue_at_end(struct deco_state *ds, struct dive *dive, const struct divecomputer *dc, deco_state_cache &cache)
{
	...
	for (auto &sample: dc->samples) {
		...
		auto [divemode, divemode_time] = d_loop.at(t0.seconds + 1);
		auto [cylinder_index, gasmix_time] = g_loop.cylinder_index_at(t0.seconds + 1);
		if (gasmix_time >= divemode_time) {
			if (cylinder_index == -1)
				divemode = OC;
			else
				divemode = get_effective_divemode(*dc, *dive->get_cylinder(cylinder_index));
		}
		...
		int depth, int *asc_cylinder, bool ccr, bool &inappropriate_cylinder_use)
{
	...
	for (size_t nr = 0; nr < gaschanges.size(); nr++) {
		...
		printf("gaschange nr %lu: @ %5.2lfm gasidx %d (%s)\n", nr, gaschanges[nr].depth / 1000.0,
...
std::vector<decostop> plan(struct deco_state *ds, struct diveplan &diveplan, struct dive *dive, int dcNr, int timestep, deco_state_cache &cache, bool is_planner, bool show_disclaimer)
{
	...
	gasmix_loop g_loop(*dive, *dc);
	divemode_loop d_loop(*dc);
	auto [divemode, divemode_time] = d_loop.at(bottom_time);
	auto [cylinder_index, gasmix_time] = g_loop.cylinder_index_at(bottom_time);
	if (cylinder_index == -1) {
		gas = gasmix_air;
		if (gasmix_time >= divemode_time)
			divemode = OC;
