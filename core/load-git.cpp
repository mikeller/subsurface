...
// AI-generated (Claude)
git_provenance loaded_git_provenance;
...
void clear_git_id()
{
	loaded_git_provenance = {};
}

void set_git_provenance(const struct git_info *info, const struct git_oid *id)
{
	char git_id_buffer[GIT_OID_HEXSZ + 1];

	git_oid_tostr(git_id_buffer, sizeof(git_id_buffer), id);
	git_provenance provenance { canonical_git_repository(info), info->branch, git_id_buffer };
	loaded_git_provenance = std::move(provenance);
}
...
static int do_git_load(const struct git_info *info, struct git_parser_state *state)
{
	...
	if (!ret) {
		set_git_provenance(info, git_commit_id(commit));
