...
bool MainWindow::saveCloudFile(const std::string &filename)
{
	...
	bool replace = preflight.status == git_save_preflight_status::replacement_confirmation_required;
	...
	showProgressBar();
	int error = replace ? replace_dives(filename.c_str()) : save_dives(filename.c_str());
	hideProgressBar();
	if (error) {
