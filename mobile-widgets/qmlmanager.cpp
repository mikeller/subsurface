...
void QMLManager::updateFirmware()
{
    QString saveFileName = ostcFirmwareCheck->getLatestFirmwareFileName();
    saveFileName = QStandardPaths::standardLocations(QStandardPaths::DownloadLocation)[0].append(QDir::separator()).append(saveFileName);
    ...
    connect(config, &ConfigureDiveComputer::progress, this, &QMLManager::displayProgress);

    ostcFirmwareCheck->updateFirmware(saveFileName, config);
}
...
void QMLManager::displayProgress(int progress)
{
	appendTextToLog(QString("update progress: %1").arg(progress));
}
